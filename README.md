...
*   份额合并与映射 (A/C Share Mapping)：自动处理同一基金 A/C/E 类份额的申购费/销售服务费差异，在回测及归因中实现底层资产的合并穿透。

### 2. 异构数据对齐与治理 
解决国内大资管跨机构数据源的异构问题：
*   多频序列对齐 (Mixed-Frequency Alignment)：针对公募日频、私募周/月频、宏观指标季频的异构数据，提供前向填充（ffill）、三次样条插值（Cubic Spline）、以及基于自回归与同类指数映射的 Beta 填充算法。
*   幸存者偏差消除 (Survivor Bias Correction)：支持底层存续期生命周期管理。引入退市/清盘基金的强制赎回机制与惩罚项，避免截面选取时的前视偏差（Look-ahead bias）。
*   基金经理职业生涯拼接 (Manager Track Splicing)：底层数据结构支持“人”与“产品”的解耦。支持将某基金经理在不同产品、不同机构的任职区间收益率进行拼接，并根据管理规模（AUM）进行加权平滑。
...
*   选股与择时能力分解：集成 T-M 模型 (Treynor-Mazuy) 与 H-M 模型 (Henriksson-Merton)，量化评估基金经理的非线性市场择时能力。

### 4. 机构级资产配置优化器
专为中低频调仓设计的组合优化算法库：
*   协方差矩阵收缩 (Covariance Shrinkage)：针对场外低频数据样本不足导致的协方差矩阵失真，内置 Ledoit-Wolf 收缩与 Oracle Approximating Shrinkage (OAS) 算法。
*   机器学习与层次组合：除传统均值方差（M-V）外，提供基于图论与无监督聚类的层次风险平价算法（HRP - Hierarchical Risk Parity）及最大分散度组合（MDP）。
